...
if ! grep -q "re.sub('-dev\$', '', dependency_value)" "$REPO_DIR/scripts/buildorder.py"; then
    echo "[1/13] Aplicando parche buildorder: -dev → padre"
    patch -p1 -d "$REPO_DIR" < "$PATCHES_DIR/001-buildorder-dev-mapping.patch"
else
    echo "[1/13] Parche buildorder ya aplicado, saltando"
fi
...
if ! grep -q 'PKG=${PKG/-dev/}' "$REPO_DIR/scripts/build/termux_extract_dep_info.sh"; then
    echo "[2/13] Aplicando parche extract_dep_info: normalización -dev"
    patch -p1 -d "$REPO_DIR" < "$PATCHES_DIR/002-extract-dep-info-dev.patch"
else
    echo "[2/13] Parche extract_dep_info ya aplicado, saltando"
fi
...
if ! grep -q "_MAJOR_VERSION:-\|_MAJOR_VERSION:-\|# Extract _MAJOR_VERSION without sourcing" "$REPO_DIR/scripts/build/termux_step_setup_variables.sh"; then
    echo "[3/13] Aplicando parche setup_variables: source tolerante"
    patch -p1 -d "$REPO_DIR" < "$PATCHES_DIR/003-setup-vars-fallback.patch"
else
    echo "[3/13] Parche setup_variables ya aplicado, saltando"
fi
...
if ! grep -q "Legacy compatibility: old build.sh files don't call termux_setup_rust" "$REPO_DIR/scripts/build/termux_step_make_install.sh"; then
    echo "[4/13] Aplicando parche make_install: termux_setup_rust automático"
    patch -p1 -d "$REPO_DIR" < "$PATCHES_DIR/004-make-install-rust.patch"
else
    echo "[4/13] Parche make_install ya aplicado, saltando"
fi
...
if ! grep -q "Legacy compatibility: extract version with grep" "$REPO_DIR/scripts/build/setup/termux_setup_rust.sh"; then
    echo "[5/13] Aplicando parche setup_rust: extraccion con grep + fallback"
    patch -p1 -d "$REPO_DIR" < "$PATCHES_DIR/005-termux-setup-rust.patch"
else
    echo "[5/13] Parche setup_rust ya aplicado, saltando"
fi
...
if ! grep -q 'Legacy compatibility: build.sh files from 2018 use "yes" instead of "true"' "$REPO_DIR/scripts/build/termux_step_start_build.sh"; then
    echo "[6/13] Aplicando parche start_build: BUILD_IN_SRC tolerante (yes/true)"
    patch -p1 -d "$REPO_DIR" < "$PATCHES_DIR/006-start-build-build-in-src.patch"
else
    echo "[6/13] Parche start_build ya aplicado, saltando"
fi
...
if ! grep -qF 'Legacy compatibility: normalize "--- ../pkg-version/..." headers in' "$REPO_DIR/scripts/build/termux_step_patch_package.sh"; then
    echo "[7/13] Aplicando parche patch_package: normalizar rutas de patches legacy"
    patch -p1 -d "$REPO_DIR" < "$PATCHES_DIR/007-patch-package-normalize-paths.patch"
else
    echo "[7/13] Parche patch_package ya aplicado, saltando"
fi
...
if ! grep -qF 'MARKER: >>> patch_package' "$REPO_DIR/build-package.sh"; then
    echo "[8/13] Aplicando parche build-package: marcadores post-patch"
    patch -p1 -d "$REPO_DIR" < "$PATCHES_DIR/008-post-patch-debug.patch"
else
    echo "[8/13] Parche build-package ya aplicado, saltando"
fi
...
if ! grep -qF 'DEBUG-MI: Cargo.toml detectado' "$REPO_DIR/scripts/build/termux_step_make_install.sh"; then
    echo "[9/13] Aplicando parche make_install: debug stderr de cargo install"
    patch -p1 -d "$REPO_DIR" < "$PATCHES_DIR/009-make-install-debug.patch"
else
    echo "[9/13] Parche make_install debug ya aplicado, saltando"
fi

# 10. Normalizar variables legacy en TODOS los build.sh (idempotente)
echo "[10/13] Normalizando variables legacy en build.sh..."
...
if ! grep -qF 'Legacy compatibility: subversioned builds allow TERMUX_PREFIX_OVERRIDE' "$REPO_DIR/build-package.sh"; then
    echo "[11/13] Aplicando parche build-package: prefix versionado (TERMUX_PREFIX_OVERRIDE)"
    patch -p1 -d "$REPO_DIR" < "$PATCHES_DIR/010-prefix-override.patch"
else
    echo "[11/13] Parche build-package prefix versionado ya aplicado, saltando"
fi
...
if ! grep -qF 'Legacy compatibility: remove --no-track for old cargo toolchains' "$REPO_DIR/scripts/build/termux_step_make_install.sh"; then
    echo "[12/13] Aplicando parche make_install: quitar --no-track del cargo install"
    patch -p1 -d "$REPO_DIR" < "$PATCHES_DIR/011-remove-cargo-no-track.patch"
else
    echo "[12/13] Parche make_install --no-track ya aplicado, saltando"
fi
...
if ! grep -qF ': "${TERMUX_PKG_LICENSE_FILE:=}"' "$REPO_DIR/scripts/build/termux_step_install_license.sh"; then
    echo "[13/13] Aplicando parche install_license: TERMUX_PKG_LICENSE* tolerantes bajo set -u"
    patch -p1 -d "$REPO_DIR" < "$PATCHES_DIR/012-license-optional.patch"
else
    echo "[13/13] Parche install_license ya aplicado, saltando"
fi
