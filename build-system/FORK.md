...
| 019 | **nuevo** | `scripts/build/termux_step_setup_variables.sh` | Shim `automake-N.N` → `automake` (del sistema): el runner (ubuntu:26.04) instala `automake` 1.17+ sin binario versionado y los tarballs históricos (tar 1.35, util-linux 2.40.2) invocan `config/missing automake-N.N` (exit 127) en remakes espurios de `Makefile.in` (maintainer-mode). Crea symlinks `automake-1.16/1.15/1.14/1.13` en `$TERMUX_TOPDIR/.prev-termux-bin` (prepended a PATH) de forma independiente del srcdir — cubre el build CI fresco (source aún sin extraer en `termux_step_setup_variables`) | `# Legacy compatibility: los tarballs historicos (tar 1.35, util-linux...` |
| 020 | **nuevo** | `scripts/build/termux_step_start_build.sh` | `TERMUX_PKG_CONFIG_LIBDIR` como **ruta única** (`$TERMUX__PREFIX__LIB_DIR/pkgconfig`, sin `:$TERMUX_PREFIX/share/pkgconfig`): ncurses 6.3 (2021) ignora `--with-pkg-config-libdir` del build.sh y usa `$PKG_CONFIG_LIBDIR` del **entorno** como dir de instalación de los `.pc`; la lista separada por `:` creaba un dir literal con `:` en el nombre, `$TERMUX_PREFIX/lib/pkgconfig` nunca se creaba y `termux_step_post_make_install` fallaba en `cd pkgconfig`. La fuente única en `termux_step_setup_pkg_config_libdir` (invocada desde `termux_step_start_build` y `termux_step_setup_toolchain`, antes del toolchain) propaga ruta única a `_29`/`_23c`/`_gnu` y al `mkdir -p`/wrapper del toolchain — el env no gana al flag | `# Legacy compatibility: ncurses 6.3 ignora --with-pkg-config-libdir y usa el env...` |
