termux_setup_toolchain_29() {
	...
	if [ "$TERMUX_ARCH" = "arm" ]; then
		...
		export GOARCH=arm64
		# Legacy compatibility: disable outline-atomics (LSE) for old packages on API 24
	...
	# Legacy compatibility: keep NDK zlib.h for old packages without zlib dep
	...
	rm usr/include/{sys/{capability,shm,sem},{glob,iconv,spawn},KHR/khrplatform,execinfo}.h
