termux_step_setup_variables() {
	...
	if [ "$TERMUX_ON_DEVICE_BUILD" = "true" ]; then
		...
		# Legacy compatibility: PrevTermux Store — permitir mover el marker al árbol montado
	...
	# Legacy compatibility: commits historicos (2018-2023) no tienen
	# packages/libllvm/build.sh ni derivan TERMUX_LLVM_VERSION. Bajo
	# set -euo pipefail un valor vacio aborta con "arithmetic syntax error".
	TERMUX_LLVM_VERSION="${TERMUX_LLVM_VERSION:-7.0.0}"
	TERMUX_LLVM_MAJOR_VERSION="${TERMUX_LLVM_VERSION%%.*}"
	TERMUX_LLVM_MAJOR_VERSION="${TERMUX_LLVM_MAJOR_VERSION:-7}"
	case "$TERMUX_LLVM_MAJOR_VERSION" in
		''|*[!0-9]*) TERMUX_LLVM_MAJOR_VERSION=7 ;;
	esac
