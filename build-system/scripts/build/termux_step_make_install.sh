...
termux_step_make_install() {
	...
	if test -f build.ninja; then
		...
		# Legacy compatibility: remove --no-track for old cargo toolchains
		...
		# Legacy compatibility: capture cargo install output to a log while
		# preserving the real exit code (PIPESTATUS[0]) for the fail-fast below.
		( cargo install \
			...
			rc=$?
			# Legacy compatibility: fail fast — propagate the real cargo install
			# exit code so the pipeline does not package an incomplete prefix.
			termux_error_exit "cargo install failed with exit $rc"
		}
	fi
}
