	...
	while read -r PKG PKG_DIR; do
		...
		if [[ "$TERMUX_INSTALL_DEPS" == "true" || "$cyclic_dependence" = "true" ]]; then
			...
			if [[ "$force_build_dependency" = "true" ]]; then
				...
				# Legacy compatibility: PrevTermux Store — reutilizar deps ya
				# compiladas del pool de releases (evita recompilar en
				# -F/subversioned cuando el pool tiene el artifact).
				if [ "${TERMUX_STORE_ENABLED:-true}" = "true" ] && \
						[ -f "$TERMUX_SCRIPTDIR/scripts/store-lib.sh" ]; then
					. "$TERMUX_SCRIPTDIR/scripts/store-lib.sh" 2>/dev/null || true
					if store_try_fetch "$PKG" "$TERMUX_ARCH" "$DEP_VERSION" "$(store_mode)"; then
						termux_add_package_to_built_packages_list "$PKG"
						[[ "$TERMUX_QUIET_BUILD" != "true" ]] && echo "Using dependency $pkg_versioned from PrevTermux Store (force build skipped)"
						continue
					fi
				fi
				...
				# Legacy compatibility: PrevTermux Store — reutilizar deps ya
				# compiladas del pool de releases (fallback silencioso al repo
				# oficial / build local si el pool no tiene el artifact).
				if [ "${TERMUX_STORE_ENABLED:-true}" = "true" ] && \
						[ -f "$TERMUX_SCRIPTDIR/scripts/store-lib.sh" ]; then
					. "$TERMUX_SCRIPTDIR/scripts/store-lib.sh" 2>/dev/null || true
					if store_try_fetch "$PKG" "$TERMUX_ARCH" "$DEP_VERSION" "$(store_mode)"; then
						termux_add_package_to_built_packages_list "$PKG"
						[[ "$TERMUX_QUIET_BUILD" != "true" ]] && echo "Using dependency $pkg_versioned from PrevTermux Store"
						continue
					fi
				fi
